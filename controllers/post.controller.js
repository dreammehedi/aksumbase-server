...
export const getPosts = async (req, res, next) => {
  try {
    ...
    if (req.query.address)
      where.address = { contains: req.query.address, mode: "insensitive" };
    ...
    if (req.query.bedrooms)
      where.bedrooms = { gte: parseInt(req.query.bedrooms) };
    if (req.query.bathrooms)
      where.bathrooms = { gte: parseInt(req.query.bathrooms) };

    // Home Type
    if (req.query.homeType)
      where.property = Array.isArray(req.query.homeType)
        ? { in: req.query.homeType }
        : req.query.homeType;
    ...
    if (req.query.minSqft)
      where.size = { ...where.size, gte: Number(req.query.minSqft) };
    if (req.query.maxSqft)
      where.size = { ...where.size, lte: Number(req.query.maxSqft) };

    // Lot Size
    if (req.query.minLotSize)
      where.lotSize = { ...where.lotSize, gte: Number(req.query.minLotSize) };
    if (req.query.maxLotSize)
      where.lotSize = { ...where.lotSize, lte: Number(req.query.maxLotSize) };

    // Year Built
    if (req.query.minYearBuilt)
      where.yearBuilt = {
        ...where.yearBuilt,
        gte: Number(req.query.minYearBuilt),
      };
    if (req.query.maxYearBuilt)
      where.yearBuilt = {
        ...where.yearBuilt,
        lte: Number(req.query.maxYearBuilt),
      };

    // HOA Fees
    if (req.query.maxHoaFees)
      where.hoaFees = { lte: Number(req.query.maxHoaFees) };
    ...
    if (req.query.moveInDate)
      where.moveInDate = { gte: new Date(req.query.moveInDate) };

    // Amenities
    if (req.query.amenities) {
      where.amenities = {
        hasEvery: Array.isArray(req.query.amenities)
          ? req.query.amenities
          : [req.query.amenities],
      };
    }
...
export const updatePost = async (req, res, next) => {
  try {
    ...
    if (user.role !== "admin" && existingPost.userId !== userId) {
      return next(
        createError(403, "You are not authorized to update this post")
      );
    }
...
export const getFlaggedPosts = async (req, res, next) => {
  try {
    const posts = await prisma.post.findMany({
      where: {
        OR: [{ flagStatus: "pending" }, { aiFlagged: true }],
      },
      include: {
        user: {
          select: {
...
export const checkForAutomatedFlags = async (post) => {
  ...
  const duplicateCheck = await prisma.post.findFirst({
    where: {
      id: { not: post.id },
      OR: [{ title: post.title }, { address: post.address }],
    },
  });
  if (duplicateCheck) {
    flags.push({
  ...
  const hasOffensiveContent = offensiveWords.some(
    (word) =>
